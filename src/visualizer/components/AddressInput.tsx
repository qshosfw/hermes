...
import { bytesToHex, hexToBytes, callsignToBytes, bytesToCallsign } from '../services/hermesProtocol';
...
const AddressInput: React.FC<AddressInputProps> = ({ label, tooltip, value, onChange, disabled = false }) => {
  ...
  const baseInputClasses = "block w-full bg-black border border-neutral-800 rounded-md py-2 px-3 text-sm text-neutral-200 focus:outline-none focus:ring-1 focus:ring-neutral-400 focus:border-neutral-400 transition-all font-mono";
  ...
  return (
    <div>
      ...
      <input
        ...
        className={`${baseInputClasses} ${!isValid ? 'border-red-500/50 text-red-400' : ''} ${disabled ? 'opacity-50 cursor-not-allowed text-neutral-500 bg-neutral-900' : ''}`}
