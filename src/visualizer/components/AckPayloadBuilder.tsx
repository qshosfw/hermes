...
import { bytesToHex, generateRandomBytes } from '../services/hermesProtocol';
...
const LqiVisualizer: React.FC<{ lqi: number }> = ({ lqi }) => {
    const percentage = (lqi / 127) * 100;
    let color = 'bg-green-500';
    if (lqi < 40) color = 'bg-red-500';
    else if (lqi < 80) color = 'bg-yellow-500';

    return (
        <div>
            <Label htmlFor="lqi" tooltip="Link Quality Indicator (0-127). A client-side metric calculated from RSSI values to estimate link reliability.">
                {`Link Quality Indicator (LQI): ${lqi}`}
            </Label>
            <div className="w-full bg-slate-700 rounded-full h-4 mt-1 relative overflow-hidden">
                <div className={`${color} h-4 rounded-full transition-all duration-300`} style={{ width: `${percentage}%` }}></div>
                <div className="absolute inset-0 flex items-center justify-center text-white text-xs font-bold mix-blend-difference">
                    {lqi} / 127
                </div>
            </div>
        </div>
...
const calculateLqi = (ackedRssi: number, ackingRssi: number, idleRssi: number): number => {
    ...
    const pF = 1 / (1 + Math.exp(-(snrF - snrMid) / snrScale));
    const pR = 1 / (1 + Math.exp(-(snrR - snrMid) / snrScale));

    const pLink = (pF + pR) / 2;

    return Math.round(pLink * 127);
};
            ...
            <div className={`p-1 -m-1 rounded-lg transition-all ${isHovered(54, 54) ? highlightClass : ''}`}>
                ...
                <div className="flex items-center justify-between pt-3">
                    <div className="flex items-center space-x-2">
                        <input id="acked-last-fragment" type="checkbox" checked={ackedPacketInfo.lastFragment} onChange={(e) => handleAckedInfoChange('lastFragment', e.target.checked)} className="h-4 w-4 rounded bg-slate-700 border-slate-600 text-blue-600 focus:ring-blue-500" />
                    ...
                    <div className="flex items-center space-x-2">
                        <input id="telemetry-bit" type="checkbox" checked={ackedPacketInfo.telemetryBit} onChange={(e) => handleAckedInfoChange('telemetryBit', e.target.checked)} className="h-4 w-4 rounded bg-slate-700 border-slate-600 text-blue-600 focus:ring-blue-500" />
                    ...
                    <div className={`transition-all duration-300 ease-in-out grid ${isTelemetrySectionExpanded ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'}`}>
                        <div className={`min-h-0 ${overflowHidden ? 'overflow-hidden' : ''}`}>
                            <div className="pt-4 space-y-4">
                                ...
                                <div className="flex items-center space-x-2">
                                    <input id="has-battery" type="checkbox" checked={ackedPacketInfo.hasBattery} onChange={(e) => handleAckedInfoChange('hasBattery', e.target.checked)} className="h-4 w-4 rounded bg-slate-700 border-slate-600 text-blue-600 focus:ring-blue-500" />
