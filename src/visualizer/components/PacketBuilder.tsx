...
const Label: React.FC<{ htmlFor: string, children: React.ReactNode, tooltip: string }> = ({ htmlFor, children, tooltip }) => (
    <label htmlFor={htmlFor} className="flex items-center space-x-2 text-xs font-medium text-neutral-400 mb-1.5 uppercase tracking-wide">
        <span>{children}</span>
        <Tooltip content={tooltip}>
            <InfoIcon className="w-3.5 h-3.5 text-neutral-600 hover:text-neutral-400 transition-colors cursor-help" />
        </Tooltip>
    </label>
);
...
const PacketBuilder: React.FC<PacketBuilderProps> = ({ 
  ...
  const baseInputClasses = "block w-full bg-black border border-neutral-800 rounded-md py-2 px-3 text-sm text-neutral-200 placeholder:text-neutral-600 focus:outline-none focus:ring-1 focus:ring-neutral-400 focus:border-neutral-400 transition-all font-mono shadow-sm";
  const checkboxClasses = "h-4 w-4 rounded border-neutral-700 bg-neutral-900 text-white focus:ring-offset-0 focus:ring-1 focus:ring-neutral-500 checked:bg-neutral-100 checked:border-transparent";
  ...
  return (
    <div className="bg-neutral-900/40 backdrop-blur-sm rounded-xl border border-neutral-800 shadow-sm overflow-hidden">
      ...
      <ConfigSection title="Header" startExpanded={true} icon={<HeaderSectionIcon />}>
        ...
        <div className="flex items-center justify-between pt-3 pb-1 border-t border-neutral-800/50 mt-2">
            <div className="flex items-center space-x-2">
                ...
                />
                <label htmlFor="want-ack" className={`text-sm text-neutral-300 cursor-pointer select-none ${isBroadcast ? 'opacity-50 cursor-not-allowed' : ''}`}>Want ACK</label>
            </div>
            <div className="flex items-center space-x-2">
                ...
                <label htmlFor="last-fragment" className="text-sm text-neutral-300 cursor-pointer select-none">Last Fragment</label>
            </div>
        </div>
