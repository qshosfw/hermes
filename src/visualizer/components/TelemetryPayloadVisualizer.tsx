...
import { TelemetryPacketInfo } from '../types';
...
const Section: React.FC<{ label: string; byteCount: number | string; children: React.ReactNode; }> = ({ label, byteCount, children }) => (
    <div className="p-4 rounded-xl border border-neutral-800 bg-neutral-900/30 space-y-3">
        <div className="flex justify-between items-baseline pb-2 border-b border-neutral-800/50">
            <h4 className="font-semibold text-xs text-neutral-400 uppercase tracking-wider">{label}</h4>
            <span className="text-[10px] font-mono text-neutral-500 bg-neutral-800 px-2 py-0.5 rounded-full">{byteCount} {typeof byteCount === 'number' && byteCount === 1 ? 'byte' : 'bytes'}</span>
        </div>
        {children}
    </div>
);
...
const ValueBlock: React.FC<{ 
    ...
    <div className="space-y-2">
        ...
        {byteSize > 4 ? (
             <div className="flex flex-wrap gap-1 bg-black/40 p-2 rounded-lg border border-neutral-800">
                {/* For large fields like location, show bits in chunks of 8 or just a representation */}
                {bits.match(/.{1,8}/g)?.map((byteBits, i) => (
                    <div key={i} className="flex gap-px">
                        {byteBits.split('').map((bit, j) => (
                             <div key={j} className={`w-1.5 h-3 ${bit === '1' ? color : 'bg-neutral-700'} opacity-80`}></div>
                        ))}
                    </div>
                ))}
             </div>
        ) : (
            <div className="flex flex-wrap justify-center items-center gap-0.5 bg-black/40 p-2 rounded-lg border border-neutral-800">
        ...
        <div className="space-y-4">
             <div className="flex justify-center items-center bg-black/40 p-3 rounded-lg border border-neutral-800">
                {flagBits.split('').map((bit, i) => (
                    <Bit key={i} value={bit} color={fields[i].color} />
                ))}
            </div>
...
const LocationBitsVisualizer: React.FC<{ loc: TelemetryPacketInfo['location'] }> = ({ loc }) => {
    const { bitFieldDefinitions, fields } = useMemo(() => {
        ...
        const bitFieldDefinitions = [
            { bits: allBitsString.substring(0, 24), color: 'bg-red-600' },
            { bits: allBitsString.substring(24, 48), color: 'bg-orange-600' },
            { bits: allBitsString.substring(48, 68), color: 'bg-amber-600' },
            { bits: allBitsString.substring(68, 78), color: 'bg-yellow-600' },
            { bits: allBitsString.substring(78, 98), color: 'bg-lime-600' },
            { bits: allBitsString.substring(98, 110), color: 'bg-green-600' },
            { bits: allBitsString.substring(110, 122), color: 'bg-emerald-600' },
            { bits: allBitsString.substring(122, 128), color: 'bg-teal-600' },
            { bits: allBitsString.substring(128, 136), color: 'bg-cyan-600' },
        ];
