...
const DataBreakdown: React.FC<{
  ...
  <div className="mt-6 animate-fade-in">
    ...
    <div className="flex w-full rounded-xl overflow-hidden shadow-2xl ring-1 ring-white/10">
      <FrameSegment label="Header" bytes={packetParts.header} color="bg-indigo-500" onClick={() => onSectionClick('Header')} />
      <FrameSegment label="Payload" bytes={packetParts.payload} color="bg-emerald-500" isClickable={false} />
      <FrameSegment label="Signature" bytes={packetParts.signature} color="bg-purple-500" isClickable={false} />
      <FrameSegment label="RS Parity" bytes={new Uint8Array(32)} color="bg-rose-500" isClickable={false} />
    </div>
  ...
  return (
    <div className="bg-neutral-900/30 p-6 rounded-xl border border-neutral-800">
      <div className="flex justify-between items-center mb-4 px-1">
        <h4 className="text-[10px] uppercase font-bold text-zinc-500 tracking-widest px-1">Physical Frame Bitstream</h4>
        <span className="font-mono text-xs text-zinc-400 bg-zinc-900 px-2 py-1 rounded-md border border-zinc-800">{totalLength} Bytes</span>
      </div>

      {/* Main Bar */}
      <div className="flex w-full rounded-xl overflow-hidden shadow-2xl ring-1 ring-white/10">
        <FrameSegment label="Preamble" bytes={preamble} color="bg-amber-500" onClick={() => onSectionClick('Preamble')} />
        <FrameSegment label="Sync Word" bytes={syncWord} color="bg-lime-500" onClick={() => onSectionClick('Sync Word')} />
        <FrameSegment label="FEC Frame" bytes={data} color="bg-indigo-500" onClick={onDataClick} />
      </div>
