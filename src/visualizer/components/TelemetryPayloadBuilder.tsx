                ...
                <div className="grid grid-cols-2 gap-3 mb-2">
                        ...
                        return (
                             <div key={flag.name} className={`flex items-center space-x-2 ${isDisabled ? 'opacity-40 cursor-not-allowed' : ''}`}>
                                <input 
                                    ...
                                    checked={isChecked} 
                                    disabled={isDisabled}
                                    onChange={(e) => handleFlagChange(flag.name as keyof TelemetryPacketInfo['flags'], e.target.checked)}
                                    className="h-4 w-4 rounded bg-slate-700 border-slate-600 text-blue-600 focus:ring-blue-500 disabled:cursor-not-allowed"
                                />
                    ...
                    <div className={`flex items-center space-x-2 ${totalUsedSpace > 21 && !telemetryInfo.flags.isCustomData ? 'opacity-40' : ''}`}>
                         <input 
                            ...
                            onChange={(e) => handleFlagChange('isCustomData', e.target.checked)}
                            className="h-4 w-4 rounded bg-slate-700 border-slate-600 text-blue-600 focus:ring-blue-500"
                        />
