...
const Section: React.FC<{ label: string; children: React.ReactNode }> = ({ label, children }) => (
    <div className="bg-zinc-900/40 border border-zinc-800/60 rounded-xl p-5 flex flex-col h-full shadow-inner">
        <h4 className="text-[10px] uppercase tracking-widest font-bold text-zinc-500 mb-6 border-b border-zinc-800 pb-2">{label}</h4>
        ...
        {byteData ? (
            <div className="space-y-1 font-mono text-sm">
                ...
                <div className="grid grid-cols-[1fr,2fr,auto] gap-x-4 items-center py-1">
                    <span className="font-sans text-zinc-400 text-xs font-medium uppercase tracking-tighter">Raw Input</span>
                    <span className="text-zinc-300 tracking-[0.2em]">{byteData.raw.toString(2).padStart(8, '0')}</span>
                    <span className="text-indigo-400 font-bold">0x{byteData.raw.toString(16).padStart(2, '0').toUpperCase()}</span>
                </div>
                ...
                <div className="mt-4">
                    <div className="grid grid-cols-[1fr,2fr,auto] gap-x-4 items-center bg-zinc-100 p-4 rounded-xl shadow-xl ring-1 ring-white/20">
                        ...
                        <span className="text-zinc-900 tracking-[0.3em] font-bold text-base">{byteData.whitened.toString(2).padStart(8, '0')}</span>
                        <span className="text-zinc-900 font-black text-lg">0x{byteData.whitened.toString(16).padStart(2, '0').toUpperCase()}</span>
                    </div>
                </div>
...
const WhiteningVisualizer: React.FC<WhiteningVisualizerProps> = ({ rawData, whitenedData, syncWord, pn15Sequence }) => {
    const [hoveredIndex, setHoveredIndex] = useState<number | null>(0);

    const selectedByteData = useMemo(() => {
        if (hoveredIndex === null || hoveredIndex >= rawData.length) return null;
        return {
            raw: rawData[hoveredIndex],
            sync: syncWord[hoveredIndex % syncWord.length],
            pn15: pn15Sequence[hoveredIndex],
            whitened: whitenedData[hoveredIndex],
        };
    }, [hoveredIndex, rawData, syncWord, pn15Sequence, whitenedData]);
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-6">
                <div className="space-y-2">
                    <h4 className="text-xs uppercase tracking-wide font-semibold text-zinc-500 pl-1">Data Frame Output</h4>
                    <HexDump
                        data={whitenedData}
                        beforeData={rawData}
                        bytesPerRow={16}
                ...
                <div className="space-y-2">
                    <h4 className="text-xs uppercase tracking-wide font-semibold text-zinc-500 pl-1">Entropy Analysis</h4>
