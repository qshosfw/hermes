...
graph TD
    ...
    B --> C{"Outer MAC Valid?<br/>(K_mesh)"}
    C -- No --> DROP["🚫 Drop Packet"]
    C -- Yes --> D["Unwrap Outer Layer"]
    D --> E{"Packet ID in Cache?"}
    E -- Yes --> DROP
    E -- No --> F["Add to Dedup Cache"]
    F --> G{"Am I Destination?"}
    G -- Yes --> H["Inner Decrypt (K_scope)"]
    G -- No --> I["Decrement TTL"]
    I --> J{"TTL = 0?"}
    J -- Yes --> DROP
    J -- No --> K["Generate New Hop Nonce"]
    K --> L["Regenerate Outer MAC"]
    L --> M["SNR-Weighted Backoff"]
    M --> N["RE-TRANSMIT"]

    classDef normal fill:#1e1e2e,stroke:#6366f1,stroke-width:1px,color:white
    classDef drop fill:#451a03,stroke:#ef4444,stroke-width:2px,color:white
    classDef good fill:#052e16,stroke:#22c55e,stroke-width:2px,color:white
    class A,B,C,D,E,F,G,I,J,K,L,M normal
    class DROP drop
    class H,N good
```

**Key property:** The Inner Layer (encrypted with $K_{scope}$) is **never touched** by routers. They only validate and regenerate the Outer Layer ($K_{mesh}$), which means routers cannot read message contents — they only verify that the packet belongs to their mesh.
