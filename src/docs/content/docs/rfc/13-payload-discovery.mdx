...
Because Discovery packets are meant to be fast and universally readable by any node running the Hermes firmware within physical RF range, they utilize the generic 56-byte envelope aggressively to pack identifying properties rather than payload text.

```mermaid
graph LR
    A["Node ID<br>(6 Bytes)"] --- B["Hardware Type<br>(1 Byte)"] --- C["Capabilities<br>(2 Bytes)"] --- D["Tx Power<br>(1 Byte)"] --- E["Available Buffer<br>(46 Bytes)"]
    classDef default fill:#1e1e2e,stroke:#89b4fa,stroke-width:2px,color:white;
    class E fill:#4c1d95,stroke:#a855f7",stroke-dasharray: 5 5,color:white;
```
...
### 13.1.2 The Available Buffer (46 Bytes)

The remaining 46 bytes are strictly formatted up to the preference of the network administrator.
...
flowchart TD
    ...
    classDef block fill:#1e1e2e,stroke:#6366f1,stroke-width:2px,color:white
