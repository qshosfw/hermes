...
graph LR
    ...
    classDef alpha fill:#1e1e2e,stroke:#3b82f6,stroke-width:2px,color:white
    classDef beta fill:#1e1e2e,stroke:#10b981,stroke-width:2px,color:white
    classDef reject fill:#451a03,stroke:#ef4444,stroke-width:2px,color:white
...
graph TD
    ...
    classDef node fill:#111827,stroke:#6366f1,stroke-width:2px,color:white
...
Direct communication paths utilize **Secret Compartmentalization** (Section 6.5) to derive session keys ($K_{scope}$) from the Master Key, the Unicast label, participant addresses, and an optional Scope Secret.

### 2.3.2 Groups (Multicast) Subnets
Groups may use any Address for identification, so long as it is stored on all participating members as a registered Multicast Destination. Group keys utilize **Secret Compartmentalization** (Section 6.5), deriving shared secrets from the Master Network Key and a group-specific Scope Secret.
