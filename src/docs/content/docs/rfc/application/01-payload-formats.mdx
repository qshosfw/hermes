...
description: High-level overview of the enclosed 56-byte sealed data structure
---

import { Package, Smartphone, Info, Lock, FileType, Zap } from 'lucide-react';

# <Package className="inline w-6 h-6 mr-2 text-blue-400" /> 1. Payload Formats

Having physically encoded the packet via FSK, protected its state via interleaved Reed-Solomon(128,96), attached the 24-byte Routing/Addressing parameters via the Transport layer, and ultimately secured it against physical tampering across the mesh via the dual-signature nested model...

...We finally land neatly down inside the **56-Byte Application Payload** shell.

## 8.1 The 56-Byte Envelope

The Hermes protocol treats these core 56 bytes as a completely agnostic, isolated envelope boundary. All contents inside are mathematically encrypted using the Packet ID and Traffic Key discussed in the security documentation.
...
graph LR
    subgraph 56-Byte Application Envelope
        direction LR
        A[Byte 0<br/>Header Type] --> B[Bytes 1-55<br/>Agnostic Application Data]
    end
    
    classDef env fill:transparent,stroke:#3b82f6,stroke-width:2px,color:inherit
    classDef data fill:transparent,stroke:#89b4fa,stroke-width:1px,color:inherit
...
graph TD
    ...
    classDef root fill:transparent,stroke:#3b82f6,stroke-width:2px,color:inherit
    classDef node fill:transparent,stroke:#6366f1,stroke-width:1px,color:inherit
    classDef dest fill:transparent,stroke:#10b981,stroke-width:2px,color:inherit
    classDef bad fill:transparent,stroke:#ef4444,stroke-width:2px,color:inherit
...
The Transport Header's `Type` integer mapping (0-15 max boundaries) explicitly outlines precisely how destination nodes should handle decoding of the internal byte arrays.

| Type Integer | Identifier | Use Case |
|---:|:---|:---|
| `0` | **ACK** | Auto-generated validations confirming Packet ID and physical transmission reception along with basic node/RSSI health parameters. |
...
| `4` | **DISCOVERY** | Used locally (via the special `DD` Discovery subnet properties) to locate neighbors and populate local neighbor tables for mesh forwarding decisions. |
...
> [!NOTE]
> Detailed specifications for ACKs have been moved to [**Application §9: Acknowledgments**](./02-acknowledgments).
